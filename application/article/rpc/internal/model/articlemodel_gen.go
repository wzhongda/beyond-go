...
var (
	...
	cacheBeyondUserArticleIdPrefix = "cache:beyondUser:article:id:"
)
...
func (m *defaultArticleModel) Delete(ctx context.Context, id uint64) error {
	beyondUserArticleIdKey := fmt.Sprintf("%s%v", cacheBeyondUserArticleIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, beyondUserArticleIdKey)
	return err
}
...
func (m *defaultArticleModel) FindOne(ctx context.Context, id uint64) (*Article, error) {
	beyondUserArticleIdKey := fmt.Sprintf("%s%v", cacheBeyondUserArticleIdPrefix, id)
	var resp Article
	err := m.QueryRowCtx(ctx, &resp, beyondUserArticleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultArticleModel) Insert(ctx context.Context, data *Article) (sql.Result, error) {
	beyondUserArticleIdKey := fmt.Sprintf("%s%v", cacheBeyondUserArticleIdPrefix, data.Id)
...
func (m *defaultArticleModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheBeyondUserArticleIdPrefix, primary)
}
