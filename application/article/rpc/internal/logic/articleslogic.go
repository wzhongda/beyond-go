...
func (l *ArticlesLogic) Articles(in *service.ArticlesRequest) (*service.ArticlesResponse, error) {
	...
	if len(articleIds) > 0 {
		...
		if err != nil {
			logx.Errorf("ArticlebyuserId userId:%d sortField:%s error:%v", in.UserId, in.SortType, err)
			return nil, err
		}
		if v == nil {
...
func (l *ArticlesLogic) articleByIds(ctx context.Context, articleIds []int64) ([]*model.Article, error) {
	articles, err := mr.MapReduce[int64, *model.Article, []*model.Article](func(source chan<- int64) {
		...
		writer.Write(p)
	}, func(pipe <-chan *model.Article, writer mr.Writer[[]*model.Article], cancel func(err error)) {
...
func articlesKey(uid int64, sortType int32) string { return fmt.Sprintf(prefixArticles, uid, sortType) }
...
func (l *ArticlesLogic) cacheArticles(ctx context.Context, uid, cursor, ps int64, sortType int32) ([]int64, error) {
	...
	pairs, err := l.svcCtx.BizRedis.ZrangebyscoreWithScoresAndLimitCtx(ctx, key, 0, cursor, 0, int(ps))
	if err != nil {
		logx.Errorf("Zrevrangebyscore key:%s,error:%v", key, err)
		return nil, err
	}
	var ids []int64
